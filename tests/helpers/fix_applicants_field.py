#!/usr/bin/env python3
"""
Fix the Applicants table field capitalization issue
...
def main():
    ...
    if not field_to_rename:
        ...
        for field in applicants_table.fields:
            if field.name == "LLM Follow-Ups":
                print("Field already has correct capitalization!")
                return
    ...
    print("Renaming to: 'LLM Follow-Ups'...")
    try:
        # Use the schema update method
        api.update_field(
            base_id,
            applicants_table.id,
            field_to_rename.id,
